...
async function main() {
  ...
  const idl = JSON.parse(fs.readFileSync("target/idl/privatemed.json", "utf-8"));
  ...
  const offset = getCompDefAccOffset("check_interaction");
  const compDefPDA = PublicKey.findProgramAddressSync(
  ...
  try {
    ...
    await uploadCircuitFallback(provider, owner, "check_interaction", program.programId, rawCircuit);
  }
